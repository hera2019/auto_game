...
BUTTON_MAIN_FG = "#888800"  # 按钮文字颜色
BUTTON_SUB_FG = "#888800"  # 按钮文字颜色
...
def place_sub_button(btnText, btnCommand, col=0, row=0):
    global colButton
    def async_command():
        button.configure(state="disabled")
        try:
...
click_btn_airship = place_main_button("飛空艇", airship_button.on_button_click)

# デイリークエスト
from dailyquest import DailyquestButton
dailyquest_button = DailyquestButton(SCALE_FACTOR)
click_btn_dailyquest = place_sub_button("デイリークエスト", dailyquest_button.on_button_click)
...
click_btn_adventure_domap = place_sub_button("Do Map", adventure_button.on_domap_click)

click_btn_adventure_battle = place_sub_button("BattleJump", adventure_button.on_battlejump_click, row=1)
...
click_btn_grandarena_battle = place_sub_button("Battle3", grandarena_button.on_battle3_click)
...
click_btn_dungeon_nextdoor = place_sub_button("Next Door", dungeon_button.on_nextdoor_click)

click_btn_dungeon_battle = place_sub_button("Battle", dungeon_button.on_battle_click, row=1)
...
click_btn_allclear = place_sub_button("Clear All", allclear_button.on_button_click)

# 定义一个函数来停止所有按钮的命令
def on_stop():
    globaldef.set_stop_loop(True)
    
click_btn_stop = place_main_button("Stop", on_stop)
